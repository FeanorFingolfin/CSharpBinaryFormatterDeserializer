...
bool BinaryFormatterDeserializer::deserialize(const QString& fileName, GrayScaleInfoBaseList& grayScaleList)
{
    // 解析规则:
    ...
    grayScaleList.clear();
    grayScaleList.reserve(grayScaleCnt);
    ...
    for (int i = 0; i < grayScaleCnt; ++i)
    {
        GrayScaleInfoBaseFileMap* grayScaleFileMap = (GrayScaleInfoBaseFileMap*)(data.data() + grayScaleDataOffset);
        GrayScaleInfoBase grayScale(grayScaleFileMap);

        int grayBitCnt = grayBitVecCntList[i]; // 获取当前 grayBitInfoList 的元素个数
        for (int j = 0; j < grayBitCnt; ++j)
        {
            GrayBitInfoFileMap* grayBit = (GrayBitInfoFileMap*)(data.data() + grayBitDataOffset);
            grayScale.grayBitInfoList.append(*grayBit);
            grayBitDataOffset += sizeof(GrayBitInfoFileMap);
            grayBitDataOffset += c_grayBitDataOffset;
        }
        grayScaleList.append(grayScale);
        grayScaleDataOffset += sizeof(GrayScaleInfoBaseFileMap);
